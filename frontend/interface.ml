...
(** let search_results p = let post_data = Yojson.Basic.from_file
    "./post_data.json" in let post_data_postboard = postboard_from_json
    post_data in let c_students_data = Yojson.Basic.from_file
    "./comments_students_data.json" in let c_students_data_list =
    comments_from_json c_students_data in let c_all_data =
    Yojson.Basic.from_file "./comments_all_data.json" in let
    c_all_data_list = comments_from_json c_all_data in let id_list =
    post_ids post_data_postboard in posts_to_string post_data_postboard
    c_students_data_list c_all_data_list (suitable_posts
    post_data_postboard id_list p)

    (** [interface ()] prompts for the location, bedrooms, and price
    range, then outputs matched posts based on the inputs. *)
    ...
    [min_bound,max_bound].\n"; print_string "> "; let p = Scanf.scanf
    "%s %i %f,%f" (fun l r p_min p_max -> input_to_query l r p_min
    p_max) in print_endline (search_results p); print_newline () *)
...
(** let rec loop () = let r = read_line () |> String.split_on_char ' '
    in let a = List.nth r 0 in let b = List.nth r 1 in let c = List.nth
    r 2 in let d = List.nth r 3 in let e = int_of_string (List.nth r 4)
    in let f = float_of_string (List.nth r 5) in let g = List.nth r 6 in
