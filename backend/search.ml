...
exception IDsUnmatched of string

let rec like_of_key_help lst id =
  match lst with
  | [] ->
      raise
        (IDsUnmatched
           "The requested post ID is not present in the likes\n  file.")
  | h :: t ->
      if h = id then string_of_int (to_int (member h likes_json))
      else like_of_key_help t id

let like_of_key id = like_of_key_help (keys likes_json) id
...
let posts_to_string_list pstbrd c_students c_all (posts : string list) =
  List.map (post_to_string pstbrd c_students c_all) posts
